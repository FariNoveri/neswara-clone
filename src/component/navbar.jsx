...
import { getAuth, signInWithEmailAndPassword, createUserWithEmailAndPassword, sendPasswordResetEmail, signInWithPopup, GoogleAuthProvider, FacebookAuthProvider, signOut, fetchSignInMethodsForEmail } from "firebase/auth";
...
const Navbar = () => {
  ...
  const verifyRecaptcha = async (token, action) => {
    try {
      console.log('Sending reCAPTCHA verification request:', { token, action });
      ...
      if (!response.ok) {
        ...
        console.error('reCAPTCHA backend error:', errorText);
      ...
      if (error.name === 'AbortError') {
        ...
        setError(error.message || 'Gagal memverifikasi reCAPTCHA. Periksa koneksi atau server.');
      }
      return false;
    }
  };
      ...
      if (err.code === "auth/email-already-in-use") {
        try {
          ...
          if (signInMethods.length > 0) {
            setError(
              "Email sudah terdaftar. Silakan login, gunakan email lain, atau reset password jika lupa kata sandi."
            );
            setFormMode("login"); // Switch to login form for convenience
          } else {
            setError("Email sudah terdaftar tetapi belum diverifikasi. Silakan cek email Anda untuk verifikasi.");
            setIsVerificationPopupOpen(true);
          }
        } catch (fetchErr) {
          ...
          >
            ...
            <form onSubmit={formMode === "login" ? handleLogin : formMode === "register" ? handleRegister : handleForgotPassword}>
              <div className="space-y-4">
                ...
                <button
                  type="submit"
                  disabled={loading || (formMode !== "forgot" && !recaptchaToken)}
